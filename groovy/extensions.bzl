...
def _version_config_setting_name(version):
    ...
    Sanitizes the version string for use as a Bazel target name (dots
    and dashes are not legal target chars; the original `4.0.32` string
    is still used as the `flag_values` value).
    """
    return "is_groovy_" + version.replace(".", "_").replace("-", "_")

def _hub_build_content(specs, testing, fetched, spock_label, default_version):
    """Render the full BUILD content for @groovy_toolchains.

    Per ISSUE-064, each registered SDK gets:

      * one `config_setting` keyed off the
        `@rules_groovy//groovy/config_settings:groovy_version` build
        flag matching that SDK's version string;
      * one `groovy_toolchain` + per-logical `groovy_deps`;
      * one `toolchain(...)` with `target_settings` pointing at the
        version's `config_setting`, so Bazel's toolchain resolver only
        matches this entry when the flag is set to that version.

    The SDK whose version equals `default_version` (i.e.
    `DEFAULT_GROOVY_VERSION`) also gets a second `toolchain(...)`
    registration gated on `:is_default` — a `config_setting` matching
    the empty-string flag value — so `bazel build //...` with no flag
    still resolves to the default toolchain.
    """
    lines = [
        "# Generated by //groovy:extensions.bzl. Do not edit by hand.",
        "",
        "load(\"@rules_groovy//groovy:defs.bzl\", \"groovy_deps\", \"groovy_toolchain\")",
        "",
    ...
    # Track which version strings we have emitted a `config_setting`
    # for. Multiple `groovy.toolchain` tags with the same version is a
    # user error elsewhere, but de-duping defensively keeps the
    # generator total.
    seen_versions = {}

    # Pick which spec receives the `:is_default`-gated `toolchain(...)`
    # registration. Preference order:
    #   1. The spec whose `version` equals `DEFAULT_GROOVY_VERSION`.
    #   2. The first declared spec.
    # The second branch matters for modules that pin a single
    # non-default version (e.g. `examples/junit4_test`'s `2.5.23`):
    # without a fallback, the no-flag case would resolve no
    # toolchain at all and every `groovy_library` build would fail.
    ...
    for i, spec in enumerate(specs):
        ...
        # The "default" spec (DEFAULT_GROOVY_VERSION-matching if
        # registered, else the first declared spec) gets a second
        # registration gated on `:is_default` so the unset-flag case
        # resolves to it. Both registrations point at the same
        # underlying `groovy_toolchain` target.
        if i == default_spec_index:
            ...
            toolchain_targets.append(":" + default_tc_target)
...
def _groovy_impl(module_ctx):
    ...
    groovy_toolchains_hub_repository(
        ...
        build_content = _hub_build_content(
            specs,
            testing,
            fetched,
            spock_label,
